...
# our
from parallel_slab.utils import generate_grid_like_pycfs, generate_regular_grid, dict_hash
...
class TestGrids:
    ...
    def test_range_of_grid(self, gen, npoints):
        ...
        sg, fg = gen(n_total_points=npoints, l_half_solid=l_half_solid, l_half_fluid=l_half_fluid)

        def test_grid(grid, max_condition):
            idx1 = grid >= 0.0  # Equals for testing some case where we exactly specify start location
    ...
    def test_one_away(self, npoints):
        ...
        # l_half_solid = total_length * 0.5
        l_half_fluid = total_length - l_half_solid
        sg, fg = generate_grid_like_pycfs(n_total_points=npoints, l_half_solid=l_half_solid, l_half_fluid=l_half_fluid)
